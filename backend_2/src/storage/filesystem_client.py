...
class FilesystemStorageClient:
    ...
    def _resolve_effective_root(self, bucket: Optional[str] = None) -> Path:
        root = self.storage_root
        bucket_value = str(bucket or "").strip()
        if not bucket_value:
            return root

        # Backward-compatible behavior: `bucket` acts like a logical subfolder.
        normalized = bucket_value.replace("\\", "/").strip("/")
        parts = [part for part in normalized.split("/") if part and part != "."]
        if any(part == ".." for part in parts):
            raise ValueError("Invalid bucket parameter")
    ...
    def resolve_object_path(self, key: str, bucket: Optional[str] = None) -> Path:
        root = self._resolve_effective_root(bucket=bucket)
    ...
    def get_object_metadata(self, key: str, bucket: Optional[str] = None) -> Dict[str, Any]:
        path = self.resolve_object_path(key, bucket=bucket)
    ...
    def open_object_stream(self, key: str, bucket: Optional[str] = None) -> BinaryIO:
        path = self.resolve_object_path(key, bucket=bucket)
        return path.open("rb")

    def get_object_range(
        self, key: str, start: int, end: int, bucket: Optional[str] = None
    ) -> bytes:
        if start < 0 or end < start:
            raise ValueError("Invalid range")

        with self.open_object_stream(key, bucket=bucket) as stream:
            stream.seek(start)
