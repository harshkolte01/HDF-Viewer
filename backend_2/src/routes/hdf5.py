...
def _call_with_optional_bucket(func, *args, bucket=None, **kwargs):
    """Call storage/reader methods with `bucket` only when explicitly provided."""
...
def _get_cached_dataset_info(reader, key, hdf_path, cache_version, bucket=None):
    ...
    dataset_info = _call_with_optional_bucket(
...
def get_children(key):
    ...
    try:
        ...
        metadata = _call_with_optional_bucket(
        ...
        children = _call_with_optional_bucket(
            reader.get_children,
            key,
            hdf_path,
            bucket=bucket
...
def get_metadata(key):
    ...
    try:
        ...
        reader = get_hdf5_reader()
        meta = _call_with_optional_bucket(
            reader.get_metadata,
            key,
            hdf_path,
            bucket=bucket
...
def get_preview(key):
    ...
    try:
        ...
        reader = get_hdf5_reader()
        preview = _call_with_optional_bucket(
            reader.get_preview,
            key,
            hdf_path,
...
def get_data(key):
    ...
    try:
        ...
        if mode == 'matrix':
            ...
            matrix = _call_with_optional_bucket(
                reader.get_matrix,
                key,
                hdf_path,
                display_dims,
            ...
            heatmap = _call_with_optional_bucket(
                reader.get_heatmap,
                key,
                hdf_path,
                display_dims,
            ...
            line = _call_with_optional_bucket(
                reader.get_line,
                key,
                hdf_path,
                display_dims,
    ...
    try:
            ...
            def generate_matrix_csv():
                ...
                for row_cursor in range(row_offset, row_end, chunk_rows):
                    ...
                    for col_cursor in range(col_offset, col_end, chunk_cols):
                        ...
                        block = _call_with_optional_bucket(
                            reader.get_matrix,
                            key,
                            hdf_path,
                            display_dims,
        ...
        def generate_line_csv():
            ...
            for cursor in range(line_offset, line_end, chunk_points):
                ...
                base_line = _call_with_optional_bucket(
                    reader.get_line,
                    key,
                    hdf_path,
                    display_dims,
                ...
                for target in compare_targets:
                    compare_line = _call_with_optional_bucket(
                        reader.get_line,
                        key,
                        target['path'],
                        display_dims,
